...
def recursive_identity_block(x_, depth, batch_norm=True, drop_rate=0.):
    global layer_idx
    ...
    x = Conv2D(depth, (3, 3), padding='same', kernel_initializer='he_normal',
               kernel_regularizer=regularizer, bias_regularizer=regularizer,
               name='conv_'+str(layer_idx))(x)
    x = Add(name='add_'+str(layer_idx))([x, x_, x__])
    x = Activation('elu', name='elu_'+str(layer_idx))(x)
    x = Dropout(drop_rate, name='dropout'+str(drop_rate)+'_'+str(layer_idx))(x)
    if batch_norm is True:
        x = BatchNormalization(scale=True, center=True, name='bn_'+str(layer_idx))(x)
...
def identity_block(x_, depth, batch_norm=True, drop_rate=0.):
    global layer_idx
...
def create_model(num_conv=4, depth=16, batch_norm=True, resnet=False, drop_rate=0., recursive=False):
    global layer_idx
    layer_idx = 1
    ...
    if resnet is False:
        ...
        if recursive is False:
            ...
            for i in range(num_conv/10):
                x = recursive_identity_block(x, depth, batch_norm, drop_rate)
...
if __name__ == '__main__':
    ...
    regularizer = l2(1e-5)
    drop_rate = 0.0
    name = 'resnet_recursive' + str(layers) + \
            '_depth' + str(depth) + \
            '_drop' + str(drop_rate)
    model_name = name + '.h5'
    ...
    with open('nohup.out.' + name, 'w') as output_file:
        ...
        for r in recursive:
            model = create_model(layers, depth=depth, batch_norm=True, resnet=True,
                    drop_rate=drop_rate, recursive=r)
